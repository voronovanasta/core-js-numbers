...
function isPowerOfTwo(num) {
  ...
  if (numItem === 1 && count !== 0) return true;
  return false;
}
...
function getRandomInteger(min, max) {
  const minItem = Math.ceil(min);
  const maxItem = Math.floor(max);
  return Math.floor(Math.random() * (maxItem - minItem + 1) + minItem);
}
